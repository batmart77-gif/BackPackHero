...
public class BackPack {
  ...
  /**
   * Attempts to place an item at the specified anchor position. * @param instance
   * The item to place.
   * 
   * @param startPos The anchor position (top-left of the item).
   * @return {@code true} if successful.
   */
  public boolean add(ItemInstance instance, Position startPos) {
    Objects.requireNonNull(instance);
    Objects.requireNonNull(startPos);
    switch (instance.getItem()) {
    case Curse c -> {
      return addCurse(instance, startPos);
    }
    default -> {
      if (!checkIfEnoughSpace(instance, startPos)) {
        return false;
      } else {
        placeItem(instance, startPos);
        return true;
      }
    }
    }
