...
/**
 * Represents a ranged weapon used for attacking enemies from a distance. This
 * weapon has an energy cost but relies on a fixed or default damage value (not
 * defined in the record fields).
 *
...
public record RangeWeapon(String name, List<Position> pos, Rarity rarity, int cost, int price) implements Item {
  ...
  public String details() {
    return "Range Weapon " + name + ", " + rarity.toString() + " needs " + cost
  ...
  public boolean use(Heros heros, Ennemi target, BackPack backpack, ItemInstance instance) {
    ...
    if (heros.depenserEnergie(cost)) {
      // 1. On cherche une flèche adjacente
      ...
      if (arrowOpt.isPresent()) {
        ...
        // 3. IMPORTANT : On consomme la flèche
